...
DoubleNode<T>* CircularList<T>::find(int index) 
{
	...
	if (index >= loc_pos)
   {
		...
		dist_prev = listSize - dist_next;  //distance using the bridge (prev refs, negative) 
   }
   else
   {
        dist_next = abs(index - loc_pos);	//distance without the bridge (prev refs, negative)
        dist_prev = listSize - dist_next;   //distance using the bridge (next refs, positive)
   }
